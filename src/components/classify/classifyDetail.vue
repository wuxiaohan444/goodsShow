...
<template>
  <div class="classifyDetail">
    ...
    <div class="shopListPrice" ref="shopListPriceAll">
      <div class="priceList">
        ...
        <div class="shopListPriceRight">
          <div ref="shopListOnePrice" v-for="(itemPrice,index) in arrPrice" :data-startPrice="itemPrice.startPrice"
               :data-endPrice="itemPrice.endPrice" @click="shopListPrice(index,$event)">{{itemPrice.text}}
          </div>
        </div>
      </div>
      <!-- 价格筛选 -->
    ...
    <div class="shopList" ref="shopListHeight">
      ...
      <button class="load_more" @click="more">{{button}}</button>
    </div>
    <!--蒙版-->
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get('/showapi/productType/getProductType')
        .then((res) => {
          if (res.data.code === 0) {
            for (let i = 0; i < res.data.data.productTypeFirstList[0].productTypeSecondList.length; i++) {
              this.arrPrice.push({
                text: res.data.data.productTypeFirstList[0].productTypeSecondList[i].priceDesc,
                startPrice: res.data.data.productTypeFirstList[0].productTypeSecondList[i].lowPrice,
                endPrice: res.data.data.productTypeFirstList[0].productTypeSecondList[i].highPrice
              })
            }
          }
        })
      ...
      console.log(IndexId, flag);
      //除了价格区间和节庆商品外 点击全部
      if (IndexId !== undefined && (flag === true || flag === 'true')) {
    ...
    methods: {
      more() {
        ...
        if (this.$route.query.flag1) {
          this.$http.get('/showapi/product/getProductList?productTypeId=' + this.$route.query.productTypeId + '&lowPrice=' + this.$route.query.startprice + '&highPrice=' + this.$route.query.endprice + '&festivalId=' + this.$route.query.festivalListId + '',{
            params: {
              page: this.page,
              rows: 10
            }
              ...
              console.log(res);
            this.shopListOne =  this.shopListOne.concat(res.data.rows);
              if (this.page == this.maxPage) {
                this.button = '没有数据了'
              }
              console.log(this.shopListOne);
        ...
        ///价格区间和节庆商品 点击全部
        if (this.$route.query.index === '1' || this.$route.query.index === 1) {
          this.$http.get('/showapi/product/getProductList', {
            params: {
              page: this.page,
              rows: 10
        ...
        } else if (this.$route.query.index === '2' || this.$route.query.index === 2) {
          this.$http.get('/showapi/product/getProductList', {
            params: {
              page: this.page,
              rows: 10
        ...
        //除了价格区间和节庆商品外 点击全部
        if (this.$route.query.index !== undefined && (this.$route.query.flag === true || this.$route.query.flag === 'true')) {
          this.$http.get('/showapi/product/getProductList?productTypeId=' + this.$route.query.index, {
            params: {
              page: this.page,
              rows: 10
            }
      ...
      shopPrice() {
        ...
        this.$refs.shopMasking.style.display = 'block';//蒙版显示于隐藏
        this.$refs.shopListHeight.style.overflow = 'hidden';
        console.log(this.$refs.shopListOnePrice.length);
        if (this.$refs.shopListOnePrice.length <= 3) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 1;
        } else if (this.$refs.shopListOnePrice.length <= 6) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 2;
        } else if (this.$refs.shopListOnePrice.length <= 9) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 3;
          console.log(shopListOne);
        } else if (this.$refs.shopListOnePrice.length <= 12) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 4;
          console.log(shopListOne);
        } else if (this.$refs.shopListOnePrice.length <= 15) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 5;
          console.log(shopListOne);
        } else if (this.$refs.shopListOnePrice.length <= 18) {
          shopListOne = (this.$refs.shopListOnePrice[0].offsetHeight + 15) * 6;
          console.log(shopListOne);
        }
